...
$(document).ready(function () {
    ...
    data.forEach((request, i) => {
      rows += `
        <tr class="hover:bg-white/5 transition">
          ...
          <td class="px-3 py-2">${new Date(request.created_at).toLocaleDateString()}</td>
        </tr>
      `;
    });
  ...
  // Load all request data initially
  ...
  $('#searchBtn').on('click', function () {
    ...
    $.get(`/api/requests/search?column=${column}&value=${value}`, function (data) {
      renderRequestTable(data);
    });
  });
});
